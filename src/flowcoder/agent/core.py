...
from flowcoder.agent.compaction import (
...
from flowcoder.agent.hook_events import (
    drain_hook_events as drain_hook_engine_events,
    run_lifecycle_hook,
)
from flowcoder.agent.llm_preparation import (
    inject_deferred_tool_reminder,
    prepare_api_conversation,
)
from flowcoder.agent.memory import AgentMemoryBridge
from flowcoder.agent.notifications import (
    consume_team_mailbox,
    inject_external_notifications,
)
from flowcoder.agent.noninteractive_tools import execute_noninteractive_tool_call
from flowcoder.agent.output_recovery import (
    OutputRecoveryState,
...
from flowcoder.agent.tool_hooks import run_post_tool_hook, run_pre_tool_hook
from flowcoder.agent.tool_results import (
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        ...
        if self.session_id:
            return str(Path(self.work_dir) / ".flowcoder" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    async def _execute_tool(
        ...
        try:
            ...
            if tc.tool_name == "AskUserQuestion":
                from flowcoder.tools.ask_user import AskUserParams
