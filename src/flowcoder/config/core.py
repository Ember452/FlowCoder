...
def _with_account_providers(config: AppConfig | None) -> AppConfig | None:
    ...
    try:
        from flowcoder.account import load_account_providers, merge_account_providers
    except Exception:
        return config
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".flowcoder" / "config.yaml",
        cwd / ".flowcoder" / "config.yaml",
        cwd / ".flowcoder" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .flowcoder/config.yaml "
            "in project or ~/.flowcoder/config.yaml, or sign in to FlowCoder Cloud"
        )
