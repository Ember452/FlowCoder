...
def is_coordinator_mode(enable_flag: bool = False) -> bool:
    if not enable_flag:
        return False
    val = os.environ.get("FLOWCODER_COORDINATOR_MODE", "").lower()
...
def match_session_mode(
    ...
    if session_is_coordinator:
        os.environ["FLOWCODER_COORDINATOR_MODE"] = "1"
    else:
...
def get_coordinator_system_prompt(agent_catalog: list[tuple[str, str]] | None = None) -> str:
    ...
    return """You are FlowCoder, an AI assistant that orchestrates software engineering tasks across multiple workers.
