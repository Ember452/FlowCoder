...
from flowcoder.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
...
from flowcoder.tools.load_skill import LoadSkill
from flowcoder.worktree.cleanup import start_stale_cleanup_task
from flowcoder.worktree.manager import WorktreeManager
from flowcoder.commands.handlers.worktree import create_worktree_command
from flowcoder.ui.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".flowcoder", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".flowcoder" / "history"
...
class FlowCoderApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_FLOWCODER_THEME)
        self.theme = "flowcoder"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                ...
                project_rules_path=Path(work_dir) / ".flowcoder" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".flowcoder" / "permissions.local.yaml",
            ),
        ...
        from flowcoder.tools.synthetic_output import SyntheticOutputTool
