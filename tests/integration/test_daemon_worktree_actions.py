...
from flowcoder.daemon.responses import DaemonActionResult
from flowcoder.daemon.session.store import SessionStore
from flowcoder.daemon.server_state import DaemonServer, DaemonSessionRuntime
from flowcoder.daemon.actions.worktree import (
...
class _WorktreeManager:
    ...
    async def create(self, name: str, base_branch: str = "HEAD"):
        ...
        worktree = Worktree(
            name=name,
            path=f"{self.root}/.flowcoder/worktrees/{name}",
...
async def test_create_and_enter_worktree_action_returns_work_dir(tmp_path):
    ...
    assert entry.work_dir.endswith("/.flowcoder/worktrees/feature")
...
def test_list_worktrees_payload_marks_current(tmp_path) -> None:
    ...
    manager.active["feature"] = Worktree(
        name="feature",
        path=f"{tmp_path}/.flowcoder/worktrees/feature",
...
async def test_create_worktree_enters_and_updates_session_work_dir(tmp_path):
    ...
    assert result.payload["status"]["work_dir"].endswith("/.flowcoder/worktrees/feature")
    assert manager.created_with == ("feature", "main")
    assert agent.work_dir.endswith("/.flowcoder/worktrees/feature")
    assert server.session_work_dir(sid).endswith("/.flowcoder/worktrees/feature")
