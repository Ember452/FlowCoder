...
from flowcoder.account.session import AccountSession
...
def test_account_sign_in_models_and_select(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with (
        ...
        patch("flowcoder.account.service.fetch_models", return_value=models),
...
def test_save_config_strips_account_providers(
    ...
    with (
        patch("flowcoder.account.service.load_account_providers", return_value=[]),
        TestClient(_app(tmp_path)) as client,
    ):
