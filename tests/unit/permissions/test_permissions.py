...
from flowcoder.permissions.dangerous import is_safe_command
...
class TestPathSandbox:
    ...
    def test_temp_dir_allowed(self) -> None:
        tmp = Path(tempfile.gettempdir()) / "flowcoder_test.txt"
...
class TestRuleEngine:
    ...
    def test_append_local_rule(self) -> None:
        tmpdir = Path(tempfile.mkdtemp())
        local_path = tmpdir / ".flowcoder" / "permissions.local.yaml"
...
class TestPermissionChecker:
    ...
    def test_plan_mode_allows_configured_plan_file(self) -> None:
        ...
        self.checker.mode = PermissionMode.PLAN
        plan_file = self.tmpdir / ".flowcoder" / "plans" / "plan.md"
    ...
    def test_plan_mode_configured_plan_file_must_stay_in_sandbox(self) -> None:
        ...
        self.checker.mode = PermissionMode.PLAN
        outside_plan = Path.home() / ".flowcoder-outside-plan.md"
    ...
    def test_plan_mode_allows_project_plan_directory(self) -> None:
        ...
        self.checker.mode = PermissionMode.PLAN
        project_plan_file = self.tmpdir / ".flowcoder" / "plans" / "other.md"
    ...
    def test_plan_mode_plan_directory_symlink_must_stay_in_sandbox(self) -> None:
        ...
        flowcoder_dir = self.tmpdir / ".flowcoder"
        flowcoder_dir.mkdir()
        plans_link = flowcoder_dir / "plans"
        try:
    ...
    def test_plan_mode_rejects_plan_directory_outside_project(self) -> None:
        ...
        self.checker.mode = PermissionMode.PLAN
        outside_plan = self.tmpdir.parent / ".flowcoder" / "plans" / "escape.md"
...
async def test_e2e_rule_allows_git():
    ...
    tmpdir = Path(tempfile.mkdtemp())
    rules_file = tmpdir / ".flowcoder" / "permissions.yaml"
