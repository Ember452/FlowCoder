...
from flowcoder.providers.anthropic_streaming import AnthropicStreamState, parse_tool_arguments
...
def test_anthropic_stream_state_accumulates_thinking() -> None:
    ...
    events += state.add_delta(SimpleNamespace(type="signature_delta", signature="sig-1"))
...
def test_anthropic_stream_state_accumulates_tool_arguments() -> None:
    ...
    start = state.start_block(SimpleNamespace(type="tool_use", name="Bash", id="tool-1"))
    events = state.add_delta(
        SimpleNamespace(type="input_json_delta", partial_json='{"command":"git')
    )
    events += state.add_delta(SimpleNamespace(type="input_json_delta", partial_json=' status"}'))
    events += state.stop_block()
