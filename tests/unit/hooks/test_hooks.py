...
from flowcoder.hooks.actions import load_action
...
class TestPromptExecutor:
    ...
    async def test_returns_message(self):
        from flowcoder.hooks.executors import execute_prompt
...
class TestHttpExecutor:
    ...
    async def test_mock_request(self):
        from flowcoder.hooks.executors import execute_http
        ...
        ctx = HookContext()
        # 用 mock 避免发起真实的网络请求
        with patch("flowcoder.hooks.executors.urlopen") as mock_urlopen:
...
class TestAgentExecutor:
    ...
    async def test_requires_configured_runner(self):
        from flowcoder.hooks.executors import execute_agent
...
class TestExecuteAction:
    ...
    async def test_dispatch(self):
        from flowcoder.hooks.executors import execute_action
...
class TestHookEngine:
    ...
    async def test_run_pre_tool_hook_exception_records_notification(self):
        ...
        with patch("flowcoder.hooks.engine.execute_action", side_effect=RuntimeError("boom")):
            result = await engine.run_pre_tool_hooks(ctx)
    ...
    async def test_async_hook_can_be_drained(self):
        ...
        with patch("flowcoder.hooks.engine.execute_action", side_effect=slow_execute):
            await engine.run_hooks("post_tool_use", ctx)
...
class TestAgentHookIntegration:
    ...
    async def test_pre_tool_use_reject_skips_tool(self):
        from flowcoder.agent import Agent, ToolResultEvent
