...
from flowcoder.teams.coordinator import (
...
class TestModels:
    ...
    def test_unique_team_name(self, tmp_dir):
        with patch("flowcoder.teams.models.Path.home", return_value=Path(tmp_dir)):
            ...
            (Path(tmp_dir) / ".flowcoder" / "teams" / "my-team").mkdir(parents=True)
...
class TestTeamManager:
    ...
    def test_get_team_returns_none_for_bad_config(self, tmp_dir):
        with patch("flowcoder.teams.models.Path.home", return_value=Path(tmp_dir)):
            team_dir = Path(tmp_dir) / ".flowcoder" / "teams" / "broken"
...
class TestBackendDetect:
    ...
    def test_no_backend_raises(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TMUX", None)
            os.environ.pop("TERM_PROGRAM", None)
            with patch("flowcoder.teams.backend_detect.shutil.which", return_value=None):
                assert detect_backend() == BackendType.IN_PROCESS
...
class TestCoordinatorMode:
    ...
    def test_enabled_with_flag_and_env(self):
        with patch.dict(os.environ, {"FLOWCODER_COORDINATOR_MODE": "1"}):
            assert is_coordinator_mode(enable_flag=True) is True

    def test_flag_without_env(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("FLOWCODER_COORDINATOR_MODE", None)
    ...
    def test_match_session_mode_switch_to_coordinator(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert os.environ.get("FLOWCODER_COORDINATOR_MODE") == "1"
...
class TestTranscript:
    ...
    def test_rejects_unsafe_agent_id(self, tmp_dir):
        from flowcoder.conversation import ConversationManager
        from flowcoder.teams.transcript import save_transcript

        with patch("flowcoder.teams.models.Path.home", return_value=Path(tmp_dir)):
            with pytest.raises(ValueError, match="agent_id must be"):
                save_transcript("test-team", "../escape", ConversationManager())

        assert not (Path(tmp_dir) / ".flowcoder" / "teams" / "escape.json").exists()
