...
from flowcoder.config import ConfigError, MemoryConfig, MemoryProviderConfig, load_config
from flowcoder.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    ToolUseBlock,
...
from flowcoder.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
...
from flowcoder.config.validator import validate_memory
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        flowcoder_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        root_md = tmp_path / "FLOWCODER.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".flowcoder"
        dotdir.mkdir()
        dot_md = dotdir / "FLOWCODER.md"
...
class TestSession:
    def test_append_writes_jsonl_and_updates_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".flowcoder" / "sessions"
...
class TestMemoryManager:
    ...
    def test_load_merges_user_and_project(
        ...
        user_mem = fake_home / ".flowcoder" / "memories.md"
        user_mem.parent.mkdir(parents=True)
        user_mem.write_text("### 用户偏好\n- prefer spaces", encoding="utf-8")

        project_mem = tmp_path / "project" / ".flowcoder" / "memories.md"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "flowcoderMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "flowcoderMd" in conv.history[0].content
...
class TestMemoryExtraction:
    def test_extraction_prompt_contains_categories(self, tmp_path: Path) -> None:
        from flowcoder.memory.auto_memory import MEMORY_EXTRACTION_PROMPT
...
class TestMemoryProviders:
    ...
    async def test_default_memory_hub_wraps_markdown_memory(
        ...
        user_mem = fake_home / ".flowcoder" / "memories.md"
        project_mem = project / ".flowcoder" / "memories.md"
        user_mem.parent.mkdir(parents=True)
        project_mem.parent.mkdir(parents=True)
    ...
    async def test_python_provider_can_be_loaded_from_config(
        ...
        (tmp_path / f"{module_name}.py").write_text(
            "from flowcoder.memory.providers import BaseMemoryProvider\n"
...
class TestMemoryConfig:
    def test_project_config_without_memory_does_not_override_home_memory(
        ...
        home = tmp_path / "home"
        project = tmp_path / "project"
        home_cfg = home / ".flowcoder" / "config.yaml"
        project_cfg = project / ".flowcoder" / "config.yaml"
