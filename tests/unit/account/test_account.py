...
from flowcoder.account.client import CatalogModel
...
from flowcoder.account.session import AccountSession, load_session, save_session
...
def test_sign_in_and_select_model(tmp_path: Path) -> None:
    ...
    with (
        patch("flowcoder.account.service.cloud_login", side_effect=fake_login),
        patch("flowcoder.account.service.fetch_models", side_effect=fake_models),
    ):
        session = sign_in(
            email="a@b.com",
            password="secret",
...
def test_load_config_injects_account_providers(tmp_path: Path, monkeypatch) -> None:
    ...
    home.mkdir()
    cwd.mkdir()
    session_file = home / ".flowcoder" / "account.yaml"
    session_file.parent.mkdir(parents=True)
    (home / ".flowcoder" / "config.yaml").write_text(
        yaml.safe_dump(
            {
                "schema_version": 1,
                "providers": [
    ...
    monkeypatch.setattr("flowcoder.config.core.Path.home", lambda: home)
    monkeypatch.setattr("flowcoder.config.core.Path.cwd", lambda: cwd)

    with patch(
        "flowcoder.account.service.fetch_models",
