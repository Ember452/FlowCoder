...
产出：`docs/reports/chaos-report.md` + 压测数据表，直接转写成简历数字。
...
> 数字标注规则：✅ = 已由混沌演练/测试实测（docs/reports/chaos-report.md，2026-08-29）；
> ⏳ = 流水线已就绪，待真实环境（Docker / LLM API key）跑出后回填，不编造数字。

- 设计 Docker 沙箱池：预热租借 + 双层超时 + 心跳对账回收，20 容器压测 0 泄漏，执行冷启动从 X s 降至 X ms ⏳（池耗尽排队/快速失败与 kill 后自愈已由混沌演练在 fake 层验证 ✅）
- 构建 HumanEval+ 评测流水线：pass@1 达 X%，自愈机制回收 X% 失败题，失败模式四分类定位改进方向 ⏳（`python -m flowcoder.eval --compare` 一键产出对比报告）
- 实现 LLM 调用治理：错误分类 + 指数退避 + 令牌桶，混沌演练（429 风暴/断网 30s/预算耗尽）下任务完成率 100% ✅（8 连击 429 全吸收、30s 断网 32.5s 内恢复收敛、超预算两阶段收敛无硬杀）
- k-sample 首胜策略：单位成本 pass@1 提升 X%（附实测曲线）⏳
- cron 驱动的自动化值守 + 仓库看门狗：事件监听触发主动建议，冷却/去重/限额门控下误报率为 X%，连续运行 X 天 0 重复推送 ⏳（P5 可选阶段，未开工）
- Outbox 式事件投递：断线重连按偏移量补投，投递不重放，压测 X 并发会话 0 丢事件 ⏳（P5c 可选阶段；P3 e2e 已覆盖 daemon 断线重连的会话状态完整性）
